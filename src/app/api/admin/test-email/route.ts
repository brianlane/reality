...
export async function POST(request: Request) {
  ...
  // First check length to prevent ReDoS attacks (RFC 5321 max length: 254)
  if (recipientEmail.length > 254) {
    return errorResponse("VALIDATION_ERROR", "Invalid email address", 400);
  }

  // Use a simpler regex that avoids polynomial time complexity
  // This regex is safe from ReDoS as it uses specific character classes
