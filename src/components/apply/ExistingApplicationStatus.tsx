...
import { STATUS_CONTENT, type StatusContentKey } from "@/lib/status-content";
...
export default function ExistingApplicationStatus({ application }: Props) {
  ...
  // Define status-specific routing/icon behavior.
  ...
  > = {
    ...
    REJECTED: {
      icon: <CopperIcon d="M6 18L18 6M6 6l12 12" />,
      actionHref: "/dashboard",
    },
    WAITLIST: {
      icon: <CopperIcon d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />,
      actionHref: "/dashboard",
    },
    WAITLIST_INVITED: {
      icon: (
  ...
  const shared = STATUS_CONTENT[displayStatus as StatusContentKey];
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
      <div className="flex flex-col items-center text-center space-y-6">
        ...
        {/* Title */}
        <h1 className="text-3xl font-semibold text-navy">{shared.title}</h1>
